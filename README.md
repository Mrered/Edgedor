...
- 面板从隐藏状态滑入，从当前侧边滑向另一侧时平滑穿越；外部点击隐藏也使用对应的滑出动画。
...
- `⌘⇧F` 或工具栏“查找”可跨所有已打开标签查找并跳转；预览标签提供手动刷新。
- 文件选择和拖放支持一次打开多个文件；标签可拖入另一个编辑分区。
- 安装后的应用可通过 Finder“打开方式 → Edgedor”接收文本、图片和 PDF 路径，并复用唯一实例。
- 设置和跨标签查找支持 `Esc` 关闭，并将焦点还给打开它们的按钮。
- 支持文本文件打开、拖放、常见 UTF-8/UTF-16/GB18030 编码和 `\n`、`\r\n`、`\r` 换行；图片和 PDF 可用预览标签打开。
- VS Code、Sublime Text、JetBrains、Vim（由 `monaco-vim` 提供编辑区模式）快捷键方案，以及“逐个选择相同内容”的图形化快捷键覆盖设置。
- 面板宽度按当前屏幕比例计算，目标范围为 20%～60%，编辑组与标签布局会随会话状态恢复。

## 已知限制

- 单个文本文件超过 20 MB 时只提示，不创建编辑标签。
- 非文本预览依赖当前 macOS WebKit 能打开的图片/PDF 数据；预览标签重启时按绑定路径重新生成数据，不支持的文件会提示后不执行操作，完整 Quick Look/Office 预览仍在后续里程碑中。
- Monaco 原生提供 VS Code 同源的多光标、矩阵选择、查找替换和编辑命令；设置中的自定义覆盖支持常用字母、数字、功能键、方向键、标点和修饰键组合。不同编辑器方案的完整工作台级快捷键不在首版承诺范围内。
